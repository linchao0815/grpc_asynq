...
protoc -I=. --proto_path=%INCLUDE_PROTO% --proto_path=%GOPATH%\pkg\mod --zap-marshaler_out=%WORK_DIR% --go_out=paths=source_relative:%WORK_DIR%\api\gen\v1 --go-asynqgen_out=paths=source_relative:%WORK_DIR%\api\gen\v1 ./user.proto
cd %WORK_DIR%
@rem pause